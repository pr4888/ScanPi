...
class GmrsTool(Tool):
    ...
    def api_router(self):
        ...
        @r.get("/conversations")
        def conversations(hours: float = 24.0, gap: float = 45.0):
            """Group TX events into back-and-forth conversations by channel + time proximity."""
            if not self._db:
                return {"conversations": [], "gap_s": gap}
            since = time.time() - hours * 3600
            rows = self._db.conn.execute(
                "SELECT id, channel, freq_hz, start_ts, end_ts, duration_s, "
                "peak_rssi, avg_rssi, ctcss_hz, ctcss_code, clip_path, "
                "transcript, transcript_status, alert_kind, alert_match "
                "FROM tx_events "
                "WHERE end_ts IS NOT NULL AND start_ts >= ? "
                "ORDER BY channel, start_ts",
                (since,),
            ).fetchall()
            # Group into conversations: same channel, gap < threshold
            from collections import defaultdict
            by_channel = defaultdict(list)
            for row in rows:
                by_channel[row["channel"]].append(dict(row))
            convs = []
            conv_id = 0
            for ch in sorted(by_channel.keys()):
                ...
                for evt in events[1:]:
                    ...
                    if evt["start_ts"] - prev_end <= gap:
                        current.append(evt)
                    else:
                        if len(current) >= 2:
                            first = current[0]
                            last = current[-1]
                            total_dur = sum(e.get("duration_s") or 0 for e in current)
                            convs.append({
                                "id": "conv_" + str(conv_id),
                                "channel": ch,
                                "start_ts": first["start_ts"],
                                "end_ts": last.get("end_ts") or last["start_ts"],
                ...
                if len(current) >= 2:
                    ...
                    conv_id += 1
            # Sort newest first
            convs.sort(key=lambda c: c["end_ts"], reverse=True)
            return {"conversations": convs, "gap_s": gap}
