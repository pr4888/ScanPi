...
class MonitorConfig:
    ...
    close_hold_s: float = 1.5  # declare CLOSE this long after RSSI first drops
    poll_hz: float = 20.0
...
class GmrsFlowgraph(gr.top_block):
    def __init__(self, channels: list[Channel], cfg: MonitorConfig):
        ...
        ch_taps = firdes.low_pass(1.0, cfg.sample_rate, 8_000, 15_000)
        ...
        for ch in channels:
            ...
            audio_squelch = analog.pwr_squelch_cc(
                cfg.squelch_db - 10.0, cfg.squelch_alpha, 0, False,
            )
