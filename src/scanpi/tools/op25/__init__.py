...
class OP25Tool(Tool):
    ...
    def api_router(self):
        ...
        @r.get("/conversations")
        def conversations(hours: float = 24.0, gap: float = 45.0, limit: int = 50):
            """Group P25 calls into conversations by talkgroup + time proximity."""
            if not self._db:
                return {"conversations": [], "gap_s": gap}
            since = time.time() - hours * 3600 if hours > 0 else 0.0
            rows = self._db.conn.execute(
                "SELECT id, tgid, tg_name, category, rid, freq_mhz, "
                "start_ts, end_ts, duration_s, clip_path, transcript, "
                "transcript_status, alert_kind, alert_match "
                "FROM p25_calls WHERE start_ts >= ? ORDER BY tgid, start_ts",
                (since,),
            ).fetchall()
            calls_by_tg = {}
            for row in rows:
                tgid = row["tgid"]
                if tgid not in calls_by_tg:
                    calls_by_tg[tgid] = []
                calls_by_tg[tgid].append(dict(row))
            convos = []
            conv_id = 0
            for tgid, calls in calls_by_tg.items():
                current = [calls[0]]
                for c in calls[1:]:
                    prev_end = current[-1].get("end_ts") or current[-1]["start_ts"]
                    if c["start_ts"] - prev_end <= gap:
                        current.append(c)
                    else:
                        if len(current) >= 2:
                            first = current[0]
                            last_ev = current[-1]
                            last_end = last_ev.get("end_ts") or last_ev["start_ts"]
                            convos.append({
                                "id": "conv_" + str(conv_id),
                                "tgid": tgid,
                                "tg_name": first.get("tg_name") or "",
                                "category": first.get("category") or "other",
                                "start_ts": first["start_ts"],
                                "end_ts": last_end,
                                "duration_s": last_end - first["start_ts"],
                ...
                if len(current) >= 2:
                    ...
                    conv_id += 1
            convos.sort(key=lambda x: x["end_ts"], reverse=True)
            return {"conversations": convos[:limit], "total": len(convos), "gap_s": gap}
