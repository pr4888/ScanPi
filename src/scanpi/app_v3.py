...
def create_app(registry: ToolRegistry, coordinator: SdrCoordinator) -> FastAPI:
    ...
    def dashboard():
        return SHELL_HTML.format(title="Dashboard", body=DASHBOARD_BODY)
    ...
    def tool_page(tool_id: str):
        ...
        page = tool.page_html()
        if page is None:
            return SHELL_HTML.format(title=tool.name, body=f"<p><em>{tool.name} has no UI page.</em></p>")
        return page
